...
for label, dataset in kbi_data.items():
    plt.plot(dataset["distance"], dataset["mddf"], label=label,
             linewidth=2, color=dataset["color"])

# --- Style the plot ---
plt.xlabel("Distance (Å)", fontsize=16, fontweight='bold')
plt.ylabel("MDDF", fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.ylim(0,13)
plt.savefig("mddf1.svg",format="svg",bbox_inches="tight",pad_inches=0.05)
plt.show()
